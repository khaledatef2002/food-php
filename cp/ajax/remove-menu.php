...
if (isset($_POST['id']) && is_logged() && check_user_perm(['manual-menu-remove'])) {
    ...
    logg("login", "لقد قام $admin بإزالة صورة منيو");

    $del = mysqli_query($GLOBALS['conn'], "DELETE FROM menu WHERE id='$id'");
    if (!$del) {
        echo "error";
    }
} else {
