...
if(isset($_POST['title']) && isset($_POST['des']) && isset($_FILES['image']) && is_logged() && check_user_perm(["main-page-cover-add"]))
{
    ...
    {
        $get_last_cover_sort = mysqli_query($GLOBALS['conn'], "SELECT * FROM main_page_header ORDER BY sort DESC");
        $sort = (mysqli_fetch_assoc($get_last_cover_sort)['sort'] ?? 0) + 1;

        mysqli_query($GLOBALS['conn'], "INSERT INTO main_page_header(sort,url, title, description) VALUES('$sort', '$msg', '$title', '$des')");
        
        $id = mysqli_insert_id($GLOBALS['conn']);

        $admin = get_admin_info()['nickname'];

        logg("main page cover", "لقد قام $admin باضافة غلاف جديد للصفحة الرئيسية بعنوان $title ومعرف $id");
        ?>
