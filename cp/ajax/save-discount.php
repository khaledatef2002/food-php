...
if (isset($_POST['discount_max_price']) && isset($_POST['id']) && isset($_POST['discount_code']) && isset($_POST['discount_name']) && isset($_POST['discount_min']) && isset($_POST['discount_max_uses']) && isset($_POST['discount_user_uses']) && in_array($_POST['active'], [0, 1, 2]) && isset($_POST['discount_start_date']) && isset($_POST['discount_start_time']) && isset($_POST['discount_end_date']) && isset($_POST['discount_end_time']) && in_array($_POST['discount_type'], array(0, 1)) && isset($_POST['discount_value']) && in_array($_POST['discount_value_type'], array(0, 1)) && in_array($_POST['location_type'], array(0, 1, 2)) && in_array($_POST['category_type'], array(0, 1, 2)) && in_array($_POST['items_type'], array(0, 1, 2)) && in_array($_POST['phone_type'], array(0, 1, 2)) && is_logged() && check_user_perm(['discounts-edit'])) {
    if (!empty($_POST['id']) || !empty($_POST['discount_code']) || !empty($_POST['discount_name']) || !empty($_POST['discount_value'])) {
        ...
        $insert = mysqli_query($GLOBALS['conn'], "UPDATE food_discounts SET max_discount='" . $discount_max_price . "', `code`='" . $discount_code . "',name='" . $discount_name . "',min_order='" . $discount_min . "',max_user_use='" . $discount_user_uses . "',max_uses='" . $discount_max_uses . "',active = '$active',start_date='" . $start . "',end_date='" . $end . "',visibility=0,locations_type='" . $location_type . "',categories_type='" . $category_type . "',items_type='" . $items_type . "',phone='" . $phone_type . "',discount_type='" . $discount_type . "',discount_value='" . $discount_value . "',discount_value_type='" . $discount_value_type . "' WHERE id=$id");

        $admin = get_admin_info()['nickname'];

        logg("login", "لقد قام $admin بتغيير بيانات كود خصم بمعرف $id");

        mysqli_query($GLOBALS['conn'], "DELETE FROM food_discounts_locations WHERE discount_id=$id");
        mysqli_query($GLOBALS['conn'], "DELETE FROM food_discounts_cat WHERE discount_id=$id");
        mysqli_query($GLOBALS['conn'], "DELETE FROM food_discounts_items WHERE discount_id=$id");
        mysqli_query($GLOBALS['conn'], "DELETE FROM food_discounts_phones WHERE discount_id=$id");
        ...
        echo json_encode(array("msg" => "success", "body" => "تم حفظ التعديلات بنجاح"));
    } else {
