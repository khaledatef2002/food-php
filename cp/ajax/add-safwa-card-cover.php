...
if (isset($_POST['title']) && isset($_POST['des']) && isset($_FILES['image']) && is_logged() && check_user_perm(['safwa-card-cover-add'])) {
    ...
    if ($msg == "FILE_SIZE_ERROR" || $msg == "FILE_TYPE_ERROR") {
        ...
        $get_last_cover_sort = mysqli_query($GLOBALS['conn'], "SELECT * FROM safwa_card_header ORDER BY sort DESC");
        $sort = (mysqli_fetch_assoc($get_last_cover_sort)['sort'] ?? 0) + 1;

        mysqli_query($GLOBALS['conn'], "INSERT INTO safwa_card_header(sort,url, title, description) VALUES('$sort', '$msg', '$title', '$des')");
        $id = mysqli_insert_id($GLOBALS['conn']);

        $admin = get_admin_info()['nickname'];

        logg("safwa card cover", "لقد قام $admin باضافة غلاف جديد لصفحة كروت الصفون بعنوان $title ومعرف $id");
?>
